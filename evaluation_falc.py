...
def cosine_distance_vect(res1, res2):
    # Extracts the scores efficiently preserving emotion order matching
    vect1 = np.array([item['score'] for item in res1])
    vect2 = np.array([item['score'] for item in res2])
...
print("Starting evaluation metrics calculation across dataset...")
...
print("Computing emotion classification embeddings...")
...
print("Computing global NLP metrics (SARI, BERTScore, ROUGE)...")
...
for idx in range(len(df)):
    ...
    score_dpo = (0.2 * sari_val) + (0.3 * bert_avg) + (0.1 * fre_score) + (0.1 * nov_score) + (0.05 * comp_ratio) + (0.25 * sim_ori_pred * 100)
